...
def regular_notifications():
    ...
    for housework_datum in housework_data:
        ...
        tdatetime = datetime.datetime.strptime(housework_datum["last_notification_date"], '%Y-%m-%d')
        last_notification_date = datetime.date(tdatetime.year, tdatetime.month, tdatetime.day)

        # Set notification date
        if housework_datum["day_week_month"] == 'd':
            reservation_date = last_notification_date + datetime.timedelta(days=housework_datum["interval"])

        elif housework_datum["day_week_month"] == 'w':
            reservation_date = last_notification_date + datetime.timedelta(days=housework_datum["interval"] * 7)

        elif housework_datum["day_week_month"] == 'm':
            reservation_date = housework_datum["last_notification_date"] + datetime.timedelta(days=housework_datum["interval"] * 31)
        ...
        if len(token_data) != 0:
            ...
            if today == reservation_date - datetime.timedelta(days=1):
                ...
                if housework_datum["day_week_month"] == 'w' or housework_datum["day_week_month"] == 'm':
                    line('Please do {} tomorrow!'.format(housework_datum["housework_name"]), token_data[0]["token"])

            # Notify if today is the reservation day.
            elif today == reservation_date:
                line('Please do {} !'.format(housework_datum["housework_name"]), token_data[0]["token"])
                # Update last notification date if "day" is registerd
                if housework_datum["day_week_month"] == 'd':
                    db.execute("UPDATE housework SET last_notification_date = ? WHERE id = ?;", reservation_date, housework_datum["id"])

            # Notify if today is the day after the reservation day.
            elif today == reservation_date + datetime.timedelta(days=1):
                if housework_datum["day_week_month"] == 'w' or housework_datum["day_week_month"] == 'm':
                    line('Did you do {} the day before? If you have not done that, you should do right now!'.format(housework_datum["housework_name"]), token_data[0]["token"])
                    # Update last notification date when "week" or "month" are registerd.
                    db.execute("UPDATE housework SET last_notification_date = ? WHERE id = ?;", reservation_date + datetime.timedelta(days=1), housework_datum["id"])
